    ...
    while True:
        ...
        if choice == "1":
            try:
                ...
                deadline = parseString(input("Enter deadline (YYYY-MM-DD HH:MM or HH:MM or YYYY-MM-DD): "))
                if deadline is None:
                    continue
            
                start_time = parseString(input("Enter start time (YYYY-MM-DD HH:MM or HH:MM or YYYY-MM-DD): "))
                if start_time is None:
                    continue

                end_time = parseString(input("Enter end time (YYYY-MM-DD HH:MM or HH:MM or YYYY-MM-DD): "))
                if end_time is None:
                    continue
            except ValueError:
                print("Invalid input. Please try again.")
                continue
            delayable_input = input("Is task absolute? (y/n): ")
